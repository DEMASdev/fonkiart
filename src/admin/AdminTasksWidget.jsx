  ...
  return (
    <div style={{ background:"#fff", border:"1px solid var(--border)", padding:"24px 28px", marginTop:2 }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
        <p style={{ fontSize:13, letterSpacing:".14em", textTransform:"uppercase", color:"var(--muted)", margin:0 }}>Pending Tasks</p>
        {remaining === 0
          ? <span style={{ fontSize:14, color:"#2d6a4f", fontWeight:500 }}>✓ All done!</span>
          : <span style={{ fontSize:14, color:"var(--muted)" }}>{remaining} remaining</span>}
      </div>
        ...
        {ADMIN_TASKS.map(t => (
          <div key={t.id} style={{ position:"relative" }}>
            <div style={{ display:"flex", alignItems:"center", gap:12, padding:"11px 14px", background:done[t.id]?"#f5f9f5":"#fdfcf8", border:`1px solid ${done[t.id]?"#c3ddc9":"var(--border)"}`, cursor:"pointer", transition:"all .15s" }}>
              ...
              <span onClick={() => handleClick(t)} style={{ fontSize:15, color:done[t.id]?"var(--muted)":"var(--ink)", textDecoration:done[t.id]?"line-through":"none", flex:1, lineHeight:1.4 }}>
                {t.text}
              </span>
              {t.action && !done[t.id] && <span onClick={() => handleClick(t)} style={{ fontSize:13, color:"var(--gold)", letterSpacing:".08em", whiteSpace:"nowrap" }}>Go →</span>}
              {!t.action && !done[t.id] && <span onClick={() => handleClick(t)} style={{ fontSize:13, color:"var(--muted)", letterSpacing:".08em", whiteSpace:"nowrap" }}>ℹ Info</span>}
            </div>
            {tooltip === t.id && t.note && (
              <div style={{ background:"#1c1a18", color:"#fff", fontSize:14, padding:"8px 14px", lineHeight:1.5 }}>{t.note}</div>
            )}
          </div>
        ))}
      </div>
