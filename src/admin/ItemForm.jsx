  ...
  return (
    <div>
      ...
      <div style={{marginBottom:16}}>
        <label style={{display:"block",letterSpacing:".1em",textTransform:"uppercase",color:"var(--muted)",marginBottom:8}}>
          Photos <span style={{fontWeight:300,textTransform:"none",letterSpacing:0}}>(click to preview)</span>
        </label>
          ...
          {form.images.map((url, i) => (
            <div key={i} style={{position:"relative",width:220,height:220,flexShrink:0,cursor:"zoom-in"}} onClick={() => setPreviewImg(url)}>
              ...
              <button onClick={e => { e.stopPropagation(); handleRemovePhoto(i); }} style={{
                ...
                lineHeight:1,display:"flex",alignItems:"center",justifyContent:"center",padding:0,zIndex:2
              }}>✕</button>
              ...
              <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0)",display:"flex",alignItems:"center",justifyContent:"center",transition:"background .2s",pointerEvents:"none"}}
                ...
                <span style={{color:"#fff",letterSpacing:".1em",opacity:0,transition:"opacity .2s"}}>🔍 Preview</span>
              </div>
            </div>
          ))}
          {form.images.length < 6 && (
            ...
            >
              {uploading ? <span style={{fontSize:28}}>⏳</span> : <>
                ...
                <span style={{letterSpacing:".1em",textTransform:"uppercase"}}>Add Photo</span>
              </>}
            </button>
          )}
        </div>
        {uploading && <p style={{color:"var(--muted)",marginTop:8}}>Uploading photo…</p>}
        {form.images.length > 1 && <p style={{color:"var(--muted)",marginTop:8}}>First photo is the main gallery image. Remove and re-add to change order.</p>}
      ...
      <div className="fld" style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <label htmlFor="isNew" style={{textTransform:"none",letterSpacing:0}}>Mark as New Collection</label>
      </div>
      <div className="fld" style={{display:"flex",alignItems:"center",gap:10}}>
        <input type="checkbox" id="isSold" checked={!!form.isSold} onChange={e=>f("isSold",e.target.checked)} style={{width:"auto"}} />
        <label htmlFor="isSold" style={{textTransform:"none",letterSpacing:0,color:"#c0392b"}}>Mark as Sold (shows "Sold Out" badge — stays visible until deleted)</label>
      </div>
      <div className="fld" style={{display:"flex",alignItems:"center",gap:10}}>
        <input type="checkbox" id="isChildren" checked={!!form.isChildren} onChange={e=>f("isChildren",e.target.checked)} style={{width:"auto"}} />
        <label htmlFor="isChildren" style={{textTransform:"none",letterSpacing:0,color:"#e74c3c"}}>❤️ Children Benefit piece (shows heart badge in catalog)</label>
      </div>
      <div style={{borderTop:"1px solid var(--border)",margin:"12px 0 12px",paddingTop:12}}>
        <p style={{letterSpacing:".1em",textTransform:"uppercase",color:"var(--muted)",marginBottom:10}}>🔑 Exclusive Collectors Room</p>
        <div className="fld" style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
          ...
          <label htmlFor="isCollectorsOnly" style={{textTransform:"none",letterSpacing:0,color:"#b8923f",fontWeight:500}}>Private Collectors Room only (hidden from public)</label>
        </div>
        <div className="fld" style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <label htmlFor="isEarlyAccess" style={{textTransform:"none",letterSpacing:0,color:"#b8923f"}}>Early Access — show to collectors before public release</label>
        </div>
      </div>
